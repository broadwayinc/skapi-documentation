...
```js
// User 'B' can now get records that require a subscription to user 'A'
skapi.getRecords({
  table: {
    name: 'Posts',
    access_group: 'authorized',
...
```js
// User 'A' blocks user 'B' from accessing subscription group 1.
skapi.blockSubscriber({
  user_id: 'user_id_of_user_B',
}).then(res=>{
  // User 'B' no longer has access to user 'A's subscription group 1.
})
```

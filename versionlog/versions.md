...
- Added the **`default_access_group`** initialization option: `new Skapi("<Project ID>", { default_access_group: 'authorized' })`. It supplies `table.access_group` for `getRecords()`, `postRecord()` and `deleteRecords()` whenever the caller omits one, so a project that keeps most of its data behind a sign-in no longer has to repeat the value on every call. It takes anything `table.access_group` takes: a number `0` to `99`, or `'public'`, `'private'`, `'authorized'`, `'admin'`. An access group written on the call itself always wins, including an explicit `0`.
- `default_access_group: 'ask'` picks no group. It makes an **omitted** `access_group` an error instead, for projects that hold data at more than one visibility and would rather fail loudly than quietly fall back to public. Calls addressed by `record_id` or `unique_id`, and updates to an existing record, carry no table at all and are unaffected in either mode.
- The project itself can carry the same value, set by its owner, and it is returned by `getConnectionInfo()` as `conf.default_access_group`. The initialization option **overrides** it, so an app that pins a value in code keeps it whatever the project setting says; when neither is set, an omitted access group behaves exactly as it always did. Note that a non-zero default makes record calls from a **signed-out** visitor throw, because unsigned users have no access to any group above `0`: such an app has to pass `access_group: 'public'` explicitly on the calls its logged-out pages make.
- See [Advanced Settings](/introduction/getting-started.html#advanced-settings).
